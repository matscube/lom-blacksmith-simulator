...
export function LayoutBasic(props: {
  ...
  return (
    <main className='flex flex-col items-center justify-between text-white'>
      ...
      {props.title && (
        <h1 className={clsx('w-full max-w-7xl', 'text-4xl text-white font-bold text-left', 'py-4')}>
          {props.title}
        </h1>
      )}
      <div className={clsx('w-full max-w-7xl', 'flex flex-col py-4 gap-y-4 text-white')}>
        {props.children}
      </div>
    </main>
  );
